...
if(length(args) < 2) {
  stop("At least two arguments must be supplied: input_dir and output_dir.", call. = FALSE)
}
input_dir <- args[1]
output_dir <- args[2]

# Adjusted to use full names from the input directory
hdf5_files <- list.files(input_dir, pattern = "\\.he5$", full.names = TRUE)

# Apply the processing function to each HDF5 file
lapply(hdf5_files, function(file_path) process_hdf5_file(basename(file_path), input_dir, output_dir))
