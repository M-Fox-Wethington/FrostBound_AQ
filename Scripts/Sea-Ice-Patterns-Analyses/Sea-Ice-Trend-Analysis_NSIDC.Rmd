...
soi_lines <- soi_lines[-c(1:2)]  # Remove header lines

# Extract and clean SOI data
soi_data <- read.table(text = soi_lines, fill = TRUE, stringsAsFactors = FALSE)
soi_data <- soi_data[complete.cases(soi_data), ]  # Remove rows with missing data

# Remove rows with non-numeric years
soi_data <- soi_data[grepl("^\\d{4}$", soi_data$V1), ]

# Extract year and monthly data
soi_years <- as.numeric(soi_data$V1)
soi_values <- soi_data[, -1]
colnames(soi_values) <- month.abb
soi_data_clean <- data.frame(Year = soi_years, soi_values)

# Convert to long format
soi_data_long <- soi_data_clean %>%
...
ggplot(correlation_results, aes(x = factor(Month, levels = 1:12), y = correlation, fill = Region)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = round(correlation, 2)), vjust = -0.5, position = position_dodge(0.9)) +
  labs(title = "Correlation between SOI and Sea Ice Extent by Region and Month",
       x = "Month", y = "Correlation") +
  theme_minimal()

# Function to fit a GLS model for a given region and month
fit_gls_model <- function(region_data) {
  gls_model <- gls(mean_extent ~ Time, correlation = corAR1(), data = region_data)
  return(gls_model)
}
