...
# Function to apply linear model to each pixel
apply_lm <- function(y) {
  if (all(is.na(y))) {
    return(NA)
  } else {
    ...
    fit <- lm(y ~ years)
    return(coef(fit)[2])  # Return the slope of the linear model
  }
}

# Apply the linear model to each pixel in the persistence stack
persistence_trend <- app(persistence_stack, apply_lm)

# Apply the linear model to each pixel in the duration stack
duration_trend <- app(duration_stack, apply_lm)

# Define a color palette with red for negative values and blue for positive values
trend_colors <- colorRampPalette(c("red", "white", "blue"))

# Plotting the trends with custom colors
plot(persistence_trend, main="Trend of Persistence Over Time", col=trend_colors(100))
plot(duration_trend, main="Trend of Duration Over Time", col=trend_colors(100))
