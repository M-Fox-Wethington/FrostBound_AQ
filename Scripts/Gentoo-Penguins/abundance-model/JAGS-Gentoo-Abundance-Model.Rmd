...
# Load Necessary Libraries
###############################################################################
library(httr)       # for GET() if not already loaded
library(dplyr)
library(tidyr)      # for pivot_longer
library(lubridate)
library(nlme)       # for mixed-effects (lme) or generalized least squares (gls)
library(readr)      # if needed for reading in .csv data
library(ggplot2)    # for quick plotting
...
###############################################################################
# 1. Load Gentoo Growth Data
###############################################################################
# This presumes you have a CSV with columns:
#   site (colony ID)
#   season (some integer or factor indicating sequential season)
#   year (calendar year, e.g. 1970 + season - 1)
#   growth_rate (mean abundance / lagged mean abundance)
# Adjust path or load method as needed.

gentoo_params <- read_csv(
  "C:/Users/michael.wethington.BRILOON/OneDrive - Biodiversity Research Institute/Documents/Manuscripts - Antarctica/FrostBound_AQ_temporary/gentoo-abundance-model/modeled_gentoo_parameters.csv"
)
str(gentoo_params)


###############################################################################
# 2. Download and Parse SOI Data Directly from NOAA
###############################################################################
...
colnames(soi_data) <- c("Year", month.abb)  # e.g., Year, Jan, Feb, Mar, ..., Dec
...
soi_data_long <- soi_data %>%
  ...
  filter(!is.na(SOI))   # remove NA rows

# Optionally, save this to an RDS for future use
saveRDS(soi_data_long, file = file.path(data_dir, "soi_data_long.rds"))
# After this point, if you wanted to skip re-downloading, you could load it back with:
# soi_data_long <- readRDS("C:/temp/soi_data_long.rds")
...
##############################################################################
# 3. Prepare Winter (June–Sept) SOI and Create Lagged Columns
###############################################################################
...
winter_soi <- soi_data_long %>%
  ...
  group_by(Year) %>%
  summarise(
    winter_soi = mean(SOI, na.rm = TRUE),
    .groups = "drop"
  )

# Create multiple lag columns (e.g., 1–5 year lags)
max_lag <- 5
winter_soi_lags <- winter_soi

for (lag_i in 1:max_lag) {
  lag_col <- paste0("winter_soi_lag", lag_i)
  # For row i (Year N), the new column is winter_soi from row i-lag_i (Year N-lag_i).
...
###############################################################################
# 4. Merge Lagged SOI Into Gentoo Growth Data
###############################################################################
# 'gentoo_params' has 'year'. We'll left_join by (year == Year).
# For row with year=1976, winter_soi_lag1 is the winter_soi from 1975, etc.
...
merged_data_complete <- merged_data %>%
  filter(!is.na(winter_soi_lag1))  # or drop any row missing any relevant lag


###############################################################################
# 5. Example Mixed-Effects Model at the Colony Level
###############################################################################
# We'll do a simple model with 1-year lag as a demonstration.
# correlation=corAR1(...) addresses time-series autocorrelation within each site.

model_lme <- lme(
  fixed = growth_rate ~ winter_soi_lag1,
  random = ~ 1 | site,
  correlation = corAR1(form = ~ year | site),  # AR(1) structure by site
  data = merged_data,
  na.action = na.omit
)
...
###############################################################################
# 6. Mixed-Effects Model with Random Slopes
###############################################################################
# If we suspect each site might respond differently to winter_soi_lag1, let slope vary by site.
# This is more flexible, but requires enough data per site to estimate slopes stably.

model_lme_slopes <- lme(
  fixed = growth_rate ~ winter_soi_lag1,
  random = ~ winter_soi_lag1 | site,
  correlation = corAR1(form = ~ year | site),
  data = merged_data,
  na.action = na.omit,
  control = lmeControl(
    opt = "optim",        # or "nlminb", or "optimx" (if installed)
    msMaxIter = 200,      # more iterations for the final step
    msVerbose = TRUE      # print progress
  )
)

summary(model_lme_slopes)


###############################################################################
# 7. Aggregated "Mean or Median" Growth Rate Approach
###############################################################################
# Some might want to see how the entire region's growth rate (mean or median) by year
# correlates with lagged SOI.

annual_growth <- merged_data %>%
  group_by(year) %>%
  summarise(
...
# Merge that with the lagged winter SOI
annual_merged <- annual_growth %>%
  left_join(winter_soi_lags, by = c("year" = "Year"))

head(annual_merged)

# Simple correlation or linear model example
cor.test(annual_merged$mean_growth, annual_merged$winter_soi_lag1, use="complete.obs")

lm_mean <- lm(mean_growth ~ winter_soi_lag1, data = annual_merged)
summary(lm_mean)

# If you want to handle autocorrelation at the annual time scale, do a GLS with AR(1):
gls_mean <- gls(
  mean_growth ~ winter_soi_lag1,
...
###############################################################################
# 8. Quick Visualization of Correlations with Multiple Lags
###############################################################################
# We'll compute correlation of each winter_soi_lagX with mean_growth.

results_list <- list()
for (lag_i in 1:max_lag) {
  colname <- paste0("winter_soi_lag", lag_i)
  tmp <- annual_merged %>%
    filter(!is.na(.data[[colname]]), !is.na(mean_growth))
  
  ctest <- cor.test(tmp[[colname]], tmp$mean_growth)
  results_list[[lag_i]] <- data.frame(
    lag    = lag_i,
    cor_est = ctest$estimate,
    p_val   = ctest$p.value
  )
}

corr_df <- do.call(rbind, results_list)
corr_df

ggplot(corr_df, aes(x = factor(lag), y = cor_est)) +
  geom_bar(stat="identity", fill="skyblue") +
  geom_text(aes(label = round(cor_est, 2)), vjust = -1.2) +
  labs(
    title = "Correlation of Mean Growth vs. Lagged Winter SOI",
    x = "Lag (Years)",
