...
**Sea Ice Trend Analysis Part 2 (Home Range Analysis)**
```{r}
...
# Define a function to count days with ice cover > ice_threshold for each year
count_ice_days <- function(x) sum(ifelse(x < ice_threshold, 0, 1), na.rm = FALSE)
...
if (length(annual_ice_days) > 0) {
  annual_ice_days_stack <- rast(annual_ice_days)
  print("Annual ice days stack created successfully.")
} else {
  stop("No valid annual ice days data to create stack.")
}

# Rename layers in the stack to match the expected format
names(annual_ice_days_stack) <- paste("Ice_Days", names(annual_ice_days_stack), sep = "_")

# Check the names of the layers in the annual ice days stack
print("Layers in annual ice days stack:")
print(names(annual_ice_days_stack))
...
calculate_open_water_days <- function(annual_ice_days_stack, years) {
  open_water_days <- sapply(years, function(y) {
    layer_name <- paste("Ice_Days", y, sep = "_")
    if (layer_name %in% names(annual_ice_days_stack)) {
      ice_days_raster <- subset(annual_ice_days_stack, layer_name)
      open_water_prop <- global(ice_days_raster, fun = function(x) mean(x < ice_threshold, na.rm = TRUE))
      return(open_water_prop)
    } else {
      print(paste("Layer not found:", layer_name))
      return(NA)
    }
  })
...
# Ensure that open_water_days is calculated correctly
