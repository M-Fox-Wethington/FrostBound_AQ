...
# Define file paths for the raster datasets
amsr_path <-"C:/Users/michael.wethington.BRILOON/OneDrive - Biodiversity Research Institute/Documents/Manuscripts - Antarctica/FrostBound_AQ/Datasets/dataset-harmonization/harmonized-dataset-comparison/amsr_common_dates.tif"
nsidc_path <- "C:/Users/michael.wethington.BRILOON/OneDrive - Biodiversity Research Institute/Documents/Manuscripts - Antarctica/FrostBound_AQ/Datasets/dataset-harmonization/harmonized-dataset-comparison/nsidc_common_dates.tif"
nsidc_harmonized_path <- "C:/Users/michael.wethington.BRILOON/OneDrive - Biodiversity Research Institute/Documents/Manuscripts - Antarctica/FrostBound_AQ/Datasets/dataset-harmonization/harmonized-dataset-comparison/nsidc_harmonized_common_dates.tif"
...
# ---- daily extent differences: harmonized vs AMSR and vs NSIDC ----------------

# Bind all daily stats and spread to wide format: one row per date, columns per Source
differences <- bind_rows(
  amsr_stats,
  nsidc_stats,
  nsidc_harmonized_stats
) %>%
  tidyr::pivot_wider(
    # all columns except Source and ice_extent_km become identifiers
    names_from  = Source,
    values_from = ice_extent_km
  ) %>%
  mutate(
    # harmonized vs AMSR (harmonized - AMSR)
    Diff_AMSR   = NSIDC_Harmonized - AMSR,
    # harmonized vs original NSIDC (harmonized - NSIDC)
    Diff_NSIDC  = NSIDC_Harmonized - NSIDC
  )

# Summaries of those differences (in km^2, same units as ice_extent_km)
diff_summary <- differences %>%
  summarise(
    # Harmonized vs AMSR (H − A)
    Mean_Diff_AMSR    = mean(Diff_AMSR,    na.rm = TRUE),
    Median_Diff_AMSR  = median(Diff_AMSR,  na.rm = TRUE),
    IQR_Diff_AMSR     = IQR(Diff_AMSR,     na.rm = TRUE),
    P5_Diff_AMSR      = quantile(Diff_AMSR, 0.05, na.rm = TRUE),
    P25_Diff_AMSR     = quantile(Diff_AMSR, 0.25, na.rm = TRUE),
    P75_Diff_AMSR     = quantile(Diff_AMSR, 0.75, na.rm = TRUE),
    P95_Diff_AMSR     = quantile(Diff_AMSR, 0.95, na.rm = TRUE),
    
    # Harmonized vs original NSIDC (H − N)
    Mean_Diff_NSIDC   = mean(Diff_NSIDC,   na.rm = TRUE),
    Median_Diff_NSIDC = median(Diff_NSIDC, na.rm = TRUE),
    IQR_Diff_NSIDC    = IQR(Diff_NSIDC,    na.rm = TRUE),
    P5_Diff_NSIDC     = quantile(Diff_NSIDC, 0.05, na.rm = TRUE),
    P25_Diff_NSIDC    = quantile(Diff_NSIDC, 0.25, na.rm = TRUE),
    P75_Diff_NSIDC    = quantile(Diff_NSIDC, 0.75, na.rm = TRUE),
    P95_Diff_NSIDC    = quantile(Diff_NSIDC, 0.95, na.rm = TRUE)
  )


# Show all columns in the console
print(diff_summary, width = Inf)

# Or, show all rows and columns (good habit with tibbles)
print(diff_summary, n = Inf, width = Inf)

# Or convert to a plain data.frame first
as.data.frame(diff_summary)
...
pub_diff_summary <- tibble(
  Comparison                           = c("Harmonized - AMSR",
                                           "Harmonized - NSIDC"),
  Mean_diff_winter_extent_km2          = c(diff_summary$Mean_Diff_AMSR,
                                           diff_summary$Mean_Diff_NSIDC),
  Median_diff_winter_extent_km2        = c(diff_summary$Median_Diff_AMSR,
                                           diff_summary$Median_Diff_NSIDC),
  IQR_diff_winter_extent_km2           = c(diff_summary$IQR_Diff_AMSR,
                                           diff_summary$IQR_Diff_NSIDC),
  P5_diff_winter_extent_km2            = c(diff_summary$P5_Diff_AMSR,
                                           diff_summary$P5_Diff_NSIDC),
  P25_diff_winter_extent_km2           = c(diff_summary$P25_Diff_AMSR,
                                           diff_summary$P25_Diff_NSIDC),
  P75_diff_winter_extent_km2           = c(diff_summary$P75_Diff_AMSR,
                                           diff_summary$P75_Diff_NSIDC),
  P95_diff_winter_extent_km2           = c(diff_summary$P95_Diff_AMSR,
                                           diff_summary$P95_Diff_NSIDC)
) %>%
  mutate(across(where(is.numeric), ~ round(.x, 0)))

print(pub_diff_summary, width = Inf)

out_dir  <- "C:/Users/michael.wethington.BRILOON/OneDrive - Biodiversity Research Institute/Documents/Manuscripts - Antarctica/FrostBound_AQ/RStudioProject/pipelines/dataset_harmonization/results"
out_file <- file.path(out_dir, "winter_extent_difference_summary_km2.csv")

write_csv(pub_diff_summary, out_file)
...
stats_plot <- stats_summary %>%
  mutate(
    ...
    Source = dplyr::recode(
      ...
      "AMSR"            = "AMSR Unified",
      "NSIDC"           = "Sea Ice Index",
      "NSIDC_Harmonized"= "Harmonized Sea Ice Index"
    ),
    # Convert to millions of km^2 for easier reading
    mean_extent   = mean_extent   / 1e6,
    median_extent = median_extent / 1e6,
    max_extent    = max_extent    / 1e6,
    min_extent    = min_extent    / 1e6
  )

# Color-blind friendly palette matching the recoded labels
line_colors <- c(
  "AMSR Unified"             = "#377eb8",
  "Sea Ice Index"            = "#ff7f00",
  "Harmonized Sea Ice Index" = "#4daf4a"
)
...
# X-axis breaks (adjust step if needed)
year_breaks <- seq(min(stats_plot$Year), max(stats_plot$Year), by = 2)

# Y-axis label used across panels
y_lab <- expression("Sea-ice extent ("*10^6*" km"^2*")")

# -------------------------------------------------------------------
# Individual panels
# -------------------------------------------------------------------

p_mean <- ggplot(stats_plot, aes(x = Year, y = mean_extent, color = Source)) +
  geom_line(linewidth = 0.5) +
  scale_color_manual(values = line_colors, name = "Dataset") +
  scale_x_continuous(breaks = year_breaks) +
  labs(
    title = "a  Mean winter extent",
    y     = y_lab,
    x     = NULL
  ) +
  theme_nature

p_median <- ggplot(stats_plot, aes(x = Year, y = median_extent, color = Source)) +
  geom_line(linewidth = 0.5) +
  scale_color_manual(values = line_colors, guide = "none") +
  scale_x_continuous(breaks = year_breaks) +
  labs(
    title = "b  Median winter extent",
    y     = y_lab,
    x     = NULL
  ) +
  theme_nature

p_max <- ggplot(stats_plot, aes(x = Year, y = max_extent, color = Source)) +
  geom_line(linewidth = 0.5) +
  scale_color_manual(values = line_colors, guide = "none") +
  scale_x_continuous(breaks = year_breaks) +
  labs(
    title = "c  Maximum winter extent",
    y     = y_lab,
    x     = "Year"
  ) +
  theme_nature

p_min <- ggplot(stats_plot, aes(x = Year, y = min_extent, color = Source)) +
  geom_line(linewidth = 0.5) +
  scale_color_manual(values = line_colors, guide = "none") +
  scale_x_continuous(breaks = year_breaks) +
  labs(
    title = "d  Minimum winter extent",
    y     = y_lab,
    x     = "Year"
  ) +
  theme_nature
...
# ------------------------------------------------------------
# Land mask (EPSG:3031, units = meters)
# ------------------------------------------------------------
...
land_mask <- rast(land_mask_path)

# ------------------------------------------------------------
# Pixel-level differences (AMSR − product) in native projection
# ------------------------------------------------------------

calculate_pixel_diff_raster <- function(layer1, layer2) {
  layer1 - layer2  # AMSR minus comparison product
}
...
# AMSR vs Harmonized Sea Ice Index
...
# ------------------------------------------------------------
# Convert to data frames in projected coordinates (meters)
# Axes will be displayed as x, y (× 10^6 m) in ggplot
# ------------------------------------------------------------

to_df_ps <- function(r) {
  df <- as.data.frame(r, xy = TRUE, na.rm = FALSE)
  names(df) <- c("x_m", "y_m", "value")
  df
}

df_land_mask <- as.data.frame(land_mask, xy = TRUE, na.rm = FALSE)
names(df_land_mask) <- c("x_m", "y_m", "mask")

# keep only land cells (assumes land mask > 0 for land; adjust condition if needed)
df_land_mask_land <- df_land_mask %>%
  filter(!is.na(mask) & mask > 0)

df_amsr_nsidc_1      <- to_df_ps(pixel_diff_amsr_nsidc_1)
df_amsr_nsidc_2      <- to_df_ps(pixel_diff_amsr_nsidc_2)
df_amsr_nsidc_3      <- to_df_ps(pixel_diff_amsr_nsidc_3)
df_amsr_harmonized_1 <- to_df_ps(pixel_diff_amsr_harmonized_1)
df_amsr_harmonized_2 <- to_df_ps(pixel_diff_amsr_harmonized_2)
df_amsr_harmonized_3 <- to_df_ps(pixel_diff_amsr_harmonized_3)

# ------------------------------------------------------------
# Color scale, theme, and axis labels
# ------------------------------------------------------------

custom_colors <- scale_fill_gradient2(
  low       = "steelblue",    # AMSR < product (SIC lower)
  mid       = "white",
  high      = "orange",       # AMSR > product (SIC higher)
  na.value  = NA,             # land handled by separate mask layer
  limits    = c(-0.5, 0.5),
  name      = "SIC difference\n(AMSR \u2212 product)"  # en dash minus; use "-" if you prefer
)
...
# Axes in units of 10^6 m
x_lab <- expression("x ("*10^6*" m)")
y_lab <- expression("y ("*10^6*" m)")

scale_xy_1e6 <- list(
  scale_x_continuous(labels = function(v) v / 1e6),
  scale_y_continuous(labels = function(v) v / 1e6)
)

# ------------------------------------------------------------
# Plots in EPSG:3031 with x,y scaled by 10^6 and black land mask
# Panel titles = dates only
# ------------------------------------------------------------

plot_amsr_nsidc_1 <- ggplot() +
  geom_raster(data = df_land_mask_land, aes(x = x_m, y = y_m), fill = "black") +
  geom_raster(data = df_amsr_nsidc_1, aes(x = x_m, y = y_m, fill = value)) +
  custom_colors +
  coord_equal() +
  scale_xy_1e6 +
  labs(
    title = format(date1, "%Y-%m-%d"),
    x = x_lab,
    y = y_lab
  ) +
  theme_map
...
plot_amsr_nsidc_2 <- ggplot() +
  geom_raster(data = df_land_mask_land, aes(x = x_m, y = y_m), fill = "black") +
  geom_raster(data = df_amsr_nsidc_2, aes(x = x_m, y = y_m, fill = value)) +
  custom_colors +
  coord_equal() +
  scale_xy_1e6 +
  labs(
    title = format(date2, "%Y-%m-%d"),
    x = x_lab,
    y = y_lab
  ) +
  theme_map
...
plot_amsr_nsidc_3 <- ggplot() +
  geom_raster(data = df_land_mask_land, aes(x = x_m, y = y_m), fill = "black") +
  geom_raster(data = df_amsr_nsidc_3, aes(x = x_m, y = y_m, fill = value)) +
  custom_colors +
  coord_equal() +
  scale_xy_1e6 +
  labs(
    title = format(date3, "%Y-%m-%d"),
    x = x_lab,
    y = y_lab
  ) +
  theme_map
...
plot_amsr_harmonized_1 <- ggplot() +
  geom_raster(data = df_land_mask_land, aes(x = x_m, y = y_m), fill = "black") +
  geom_raster(data = df_amsr_harmonized_1, aes(x = x_m, y = y_m, fill = value)) +
  custom_colors +
  coord_equal() +
  scale_xy_1e6 +
  labs(
...
plot_amsr_harmonized_2 <- ggplot() +
  geom_raster(data = df_land_mask_land, aes(x = x_m, y = y_m), fill = "black") +
  geom_raster(data = df_amsr_harmonized_2, aes(x = x_m, y = y_m, fill = value)) +
  custom_colors +
  coord_equal() +
  scale_xy_1e6 +
  labs(
...
plot_amsr_harmonized_3 <- ggplot() +
  geom_raster(data = df_land_mask_land, aes(x = x_m, y = y_m), fill = "black") +
  geom_raster(data = df_amsr_harmonized_3, aes(x = x_m, y = y_m, fill = value)) +
  custom_colors +
  coord_equal() +
  scale_xy_1e6 +
  labs(
...
col_title_right <- ggplot() +
  theme_void() +
  ggtitle("AMSR − Harmonized Sea Ice Index") +
  theme(
    plot.title = element_text(
      hjust = 0.5,
      face  = "bold",
...
# ------------------------------------------------------------
# Combined panel: column titles + maps
# ------------------------------------------------------------

combined_plot <-
  (col_title_left + col_title_right) /
  (plot_amsr_nsidc_1 + plot_amsr_harmonized_1) /
...
# Adjust these numbers to whatever you were using before, × 1.1 for +10%
ggsave(
  filename = "Figure4_pixel_differences.png",
  plot     = combined_plot,
  width    = 7.7,   # was 7.0
  height   = 9.9,   # was 9.0
  units    = "in",
...
ggsave(
  filename = "Figure4_pixel_differences.pdf",
  plot     = combined_plot,
  width    = 7.7,
  height   = 9.9,
  units    = "in"
